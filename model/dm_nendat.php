...
class DoanhMucNenDat extends Database{
     ...
     * Update lo dat by id
     */
    function update($id, $id_khudat, $id_lodat, $ten_nendat, $lo_gioi, $chieu_dai,$chieu_rong,$dien_tich,$dien_tich_xd,$gia,$so_tang,$mo_ta,$image,$hien_trang,$id_hd){
        $sql = parent::$connection->prepare("UPDATE `dm_nendat` SET `id_khudat`= ?,`id_lodat`= ?,`ten_nendat`= ?,`lo_gioi`= ?,`chieu_dai`= ?,`chieu_rong`= ?,`dien_tich`= ?,`dien_tich_xd`= ?,`gia`= ?,`so_tang`= ?,`mo_ta`= ?,`image`= ?,`hien_trang`= ?,`id_hd`= ? WHERE id = ?");
        $sql->bind_param('iisiiiiiiisssii', $id_khudat, $id_lodat, $ten_nendat, $lo_gioi, $chieu_dai,$chieu_rong,$dien_tich,$dien_tich_xd,$gia,$so_tang,$mo_ta,$image,$hien_trang,$id_hd, $id);
        return $sql->execute();
    }
    /**
     * Delete lo dat by Id
